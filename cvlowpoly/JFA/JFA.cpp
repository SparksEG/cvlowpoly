...
void ExecuteJumpFloodingDis(float* flowDisMap, Seeds Seeds, unsigned int xsize, unsigned int ysize) {
	...
	int* disMap = (int*)malloc(sizeof(Pixel) * xsize * ysize);
			...
			for (int x = 0; x < BufferWidth; ++x) {
				...
				for (int ky = -1; ky <= 1; ++ky) {
					for (int kx = -1; kx <= 1; ++kx) {				//当前点（已被标记）的上下左右
						...
						int newDist = (pk.x - x)*(pk.x - x) + (pk.y - y)*(pk.y - y);		//新的距离是  当前点（x，y） 和 临近点pk
	...
	for (unsigned int i = 0; i < xsize*ysize; i++) {
		...
		if ((tparm % 2)) {
			...
			flowDisMap[i] = float((distmap[i] % minlen) * 255) / minlen;
		}
		else {

			flowDisMap[i] = float(1 - (distmap[i] % minlen)) * 255 / minlen;
		}
	}

...
int EDLineDetector::float_equal(float a, float b)
{
...
float EDLineDetector::log_gamma_lanczos(float x)
{
...
float EDLineDetector::log_gamma_windschitl(float x)
{
	return 0.918938533204673f + (x - 0.5f)*log(x) - x
		+ 0.5f*x*log(x*sinh(1 / x) + 1 / (810.f*pow(x, 6.f)));
}
/** Computes -log10(NFA).*/
float EDLineDetector::nfa(int n, int k, float p, float  logNT)
{
...
int EDLineDetector::EdgeDrawing(image_int8u_p image, EdgeChains &edgeChains, bool smoothed)
{
	...
	unsigned int edgePixelArraySize = pixelNum / 5;//5
	unsigned int maxNumOfEdge = edgePixelArraySize / 20;//20
...
EdgeChains GetEdgeChains(unsigned char* data, unsigned int xsize, unsigned int ysize) {
	...
	if (EDL->EdgeDrawing(image, edges)) {
		printf("Line Detection is not finished\n");
		return edges;
	}
	else {
